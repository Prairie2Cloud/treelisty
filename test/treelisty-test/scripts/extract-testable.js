...
function main() {
    console.log('🔧 Extracting testable modules from treeplexity.html...\n');
    ...
    if (!scriptMatches) {
        console.error('❌ No <script> tags found in source file');
        process.exit(1);
    }
    ...
    console.log(`📄 Extracted ${jsCode.length.toLocaleString()} characters of JavaScript\n`);
...
`;
    ...
    for (const constName of CONSTANTS_TO_EXTRACT) {
        ...
        if (constMatch) {
            ...
            console.log(`✅ Extracted constant: ${constName}`);
        } else {
            console.log(`⚠️  Constant not found: ${constName}`);
        }
    }
...
`;
        console.log(`📝 Stub created for: ${constName}`);
    }
...
`;
            console.log(`⚠️  Function not found (stub created): ${funcName}`);
        }
    }
